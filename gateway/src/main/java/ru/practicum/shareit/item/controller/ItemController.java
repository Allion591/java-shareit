...
public class ItemController {
    ...
    @PostMapping
    public ResponseEntity<Object> add(@RequestHeader("X-Sharer-User-Id") @Min(1) Long userId,
    ...
    @GetMapping("/{itemId}")
    public ResponseEntity<Object> getById(@RequestHeader("X-Sharer-User-Id") @Min(1) Long userId,
                                          @PathVariable @Min(1) Long itemId) {
    ...
    @GetMapping
    public ResponseEntity<List<Object>> get(@RequestHeader("X-Sharer-User-Id") @Min(1) Long userId) {
        log.info("Шлюз принял запрос на получение всех вещей пользователя: {}", userId);
        return itemClient.getItems(userId);
    }

    @GetMapping("/search")
    public ResponseEntity<List<Object>> search(@RequestParam @NotNull String text,
    ...
    public void deleteItem(@RequestHeader("X-Sharer-User-Id") @Min(1) Long userId,
